...
1. 打开工具，在「输入区域」粘贴原始 iframe 标签

2. 点击「格式化链接（自动复制）」按钮

3. 提示框显示「格式化完成」后，直接在目标位置粘贴（结果已自动复制）

4. 如需重新操作，点击「清空内容」即可重置输入 / 输出区域
...
3. 打包完成后，在 dist 目录中找到 .exe 文件，双击即可运行

## ⚠️ 注意事项

1. 仅支持处理完整的 iframe 标签，若输入不完整（如缺少 </iframe> 结尾），会提示「未找到有效的 src 属性值」

2. src 属性值需以 // 开头（如 //[player.bilibili.com/xxx](http://player.bilibili.com/xxx)），工具会自动补全 https:

3. 若原始标签中已存在 autoplay 参数（如 autoplay=1），工具会自动替换为 autoplay=0

4. 工具会保留原始标签中的其他属性（如 allowfullscreen、scrolling 等），不会删除或修改
